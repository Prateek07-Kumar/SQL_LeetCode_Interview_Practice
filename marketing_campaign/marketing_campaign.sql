...
You have a table of in-app purchase by user. Users that make their first in-app purchase are place
in a marketing campaign where they see call-to-actions for more in-app purchases.
Find the nuber of users that made additional in-app purchases dur to the success of the marketing campaign.

The marketing campaign doesn;t start untile one dat after the initial in-app purchase so users that only
mase one or multiple purchese on the first day do not count, nor do we count users that over ime purchase
only the product they purchases on the first day.*/
...
left join first_app_purchases b on a.user_id=b.user_id and a.product_id=b.product_id
